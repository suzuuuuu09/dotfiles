return {
	...
	config = function()
		...
		vim.api.nvim_set_hl(0, "TabLine", { bg = darker_bg, fg = utils.get_highlight("Comment").fg })
		vim.api.nvim_set_hl(0, "TabLineFill", { bg = normal_bg })
		vim.api.nvim_set_hl(0, "TabLineSel", { bg = c.polar_night.brightest, fg = c.snow_storm.origin, bold = true })
		...
		-- インデックス番号の修正: self._index が nil の場合のフォールバックを追加
		local TablineIndex = {
			provider = function(self)
				local idx = self._index or 0
				return idx > 0 and (tostring(idx) .. ": ") or ""
			end,
			hl = { fg = c.frost.ice_water, bold = true },
		}
		...
		local TablineGitStatus = {
			condition = function(self)
				local stats = vim.b[self.bufnr].gitsigns_status_dict
				return stats and ((stats.added or 0) + (stats.changed or 0) + (stats.removed or 0) > 0)
			end,
			provider = " ",
			hl = { fg = c.frost.polar_night_light },
		}
		...
		local TablineFileFlags = {
			{
				...
				provider = " " .. (icons.modified_icon or "●"),
		...
		local TablineCloseButton = {
			...
			{
				provider = " " .. (icons.close_icon or "󰅖"),
