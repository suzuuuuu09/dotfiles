...
darwin-rebuild switch --flake ~/dotfiles#suzuMac

# Or use the shorthand (if nix is in PATH)
nix run nixpkgs#darwin-rebuild -- switch --flake ~/dotfiles#suzuMac
```

The flake.nix is located at `~/dotfiles/flake.nix` (repo root).
...
- **`.config/nix/`** - Main Nix module/config directory
